...
<body class="hold-transition skin-yellow layout-top-nav">
                        ...
                        <div class="container-fluid">
                            ...
                            <input type="hidden" name="id" value="">
                                <div class="form-group mb-2">
                                    ...
                                    <select class="form-control form-control-sm rounded-0" name="faculty" id="faculty_id" required onchange="loadCalendarEvents(this.value)" required>
                                        <option value="">Select Faculty</option>
                                        <?php
                                        $faculties = $con->query("SELECT * FROM `faculty`");
                                        while ($row = $faculties->fetch_assoc()) {
                                            $facultyOption = $row['id_no'] . ' - ' . $row['firstname'];
                                            echo "<option value=\"{$row['id_no']}\">{$facultyOption}</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
    ...
    <?php
    $schedules = $con->query("SELECT id, title, description, start_datetime, end_datetime FROM `schedule_list`");
    $sched_res = [];
...
<script>
    ...
    var calendar = new Calendar(calendarEl, { events: [ 
